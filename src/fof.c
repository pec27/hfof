		   ...
		   const int64_t *restrict sort_idx, int32_t *restrict domains)
{
  ...
  */
  ...
  const int N2 = N*N;
  const int64_t walk_ngbs[58] = {N2, 1, N, N2+N, N2-N, N2-1, N2+1, N+1, N-1, 
				 N2-N+1, N2+N+1, N2-N-1, N2+N-1, 2*N2, 2*N, 2, 
				 2*N2+N, N2+2*N, 2*N2-N, N2-2*N, N+2, N-2, N2+2, 
				 2*N2+1, N2-2, 2*N-1, 2*N+1, 2*N2-1, 2*N2-N-1, 
				 2*N2+N-1, N2+2*N-1, N2-2*N+1, N2+N-2, 2*N2-N+1, 
				 2*N2+N+1, N2+2*N+1, N2-N+2, N2+N+2, N2-N-2, 
				 N2-2*N-1, 2*N2+2*N, 2*N-2, 2*N2-2*N, 2*N2-2, 
				 2*N2+2, 2*N+2, 2*N2+N-2, 2*N2-N-2, N2+2*N+2, 
				 2*N2-2*N-1, 2*N2+2*N-1, 2*N2-2*N+1, 
				 2*N2+2*N+1, N2-2*N+2, 2*N2-N+2, 2*N2+N+2, 
				 N2+2*N-2, N2-2*N-2};
	      ...
	      if (adj_root!=my_root) 
		{
		  ...
		  */
		  for (int my_k=cur_start;my_k<cur_end;++my_k)
		    {
		      const int p1 = sort_idx[my_k];
		      const double x1 = xyz[p1*3], y1 = xyz[p1*3+1], z1 = xyz[p1*3+2];
		      for (int adj_k=cell_start_end[adj_idx],p2=sort_idx[adj_k];
  ...
  */
  ...
  for (unsigned int i=0;i<num_cells;++i)
      ...
      for (int k=cur_start;k<cur_end;++k)
	{
	  ...
	  unsigned int j=((unsigned int)orig_cell*hprime)&hmask;
	  while ((~htable[j].icell)!=orig_cell)
	    j=(j+1)&hmask;
	  const int adj_idx = htable[j].idx;
