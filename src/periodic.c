...
{
  /*
    Scale every position into the unit square and find the image in [0,1) and
    append it into [1, 1+r_pad*inv_width)
    
   */
  const double pad = r_pad * inv_width; // Scaled r_pad
  int image = 0; // image to add

  double *new_pos = &pad_pos[(size_t)num_pos*2];
  const double *im_start = new_pos;
  ...
  for (size_t i=0; i<num_pos;++i, im_start = new_pos)
    {
      ...
      const double ux = pos[i*2]*inv_width,
	uy = pos[i*2+1]*inv_width;
